...
Template.Company_edit.onRendered(function() {
  ...
  instance.$('#nameInput').val(instance.data.company.name);
  instance.$('#countryInput').val(CountryCodes.countryName(instance.data.company
    .country)).change();
  instance.$('#cuitInput').val(instance.data.company.fin);
  ...
  instance.$('[data-action=form]').validate({
    ...
    submitHandler: () => {
      ...
      const country = CountryCodes.countryCode($('#countryInput').val());
      const cuit = $('#cuitInput').val();
      const companyId = instance.data.company._id;
      Companies.update({
        _id: companyId
      }, {
