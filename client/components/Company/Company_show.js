Template.Company_show.onCreated(function() {
  this.autorun(() => {
    this.subscribe('Actodes.byId', FlowRouter.getParam('_id') );
  });
})
...
Template.Company_show.events({
  'click [data-action=showPlace_create]': function() {
    ...
    BlazeLayout.render('App_body', {main: 'Company_show', create:'Place_create'});
    Session.set('creating', true);
  },
  'click [data-action=showContact_create]': function() {
    //console.log(e.target.id);
    BlazeLayout.render('App_body', {main: 'Company_show', create:'Contact_create'});
    Session.set('creating', true);
  },
