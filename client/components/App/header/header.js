...
Template.header.events({
  ...
  'keyup #search-input': function(evt) {
    var kids = $('#search-result-ul').children();
    var selected = $('#search-result-ul').children('.h-bg-yellow');
    ...
    if (evt.keyCode == '39' || evt.keyCode == '40') {
      ...
      if (selIndex < (len - 1)) {
        ...
        selected.next().addClass('h-bg-yellow');
        /*console.log("down", kids.index(selected), len);*/
      } else {
        //bottom reached
        selected.removeClass('h-bg-yellow');
        kids.first().addClass('h-bg-yellow');
        /*console.log("down", kids.index(selected), len);*/
      }
    } else {
      //other key
      if (evt.keyCode == 13) {
        //enter
        var url = selected.children().first().attr('href');
        ...
        if (input.val().length < 1 && evt.keyCode == '8') {
          hideSearchResults();
        } else {
          if (evt.keyCode == '8') {
            //backspace
